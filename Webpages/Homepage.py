...
def md_generator():
    sbar = st.sidebar
    with sbar:
        st.markdown('''### Tutorial 📖 [here](https://www.realpythonproject.com/a-free-tool-to-take-your-github-profile-to-the-next-level/)''')

    st.subheader("Fill in the text boxes and click on Generate README")
    # ---- github addons theme list ---- #
    theme_list = ["default", "solarized-light", "dark", "radical", "merko", "gruvbox", "tokyonight", "onedark",
                  "cobalt",
                  "synthwave", "highcontrast", "dracula"]

    github_stats_type = sbar.selectbox(label="Chose Github Stats Card Type", options=['type-1', 'type-2', 'type-3'],
                                       index=1)
    ...
    user_skills = st.multiselect("Select Skills", options=skills,
                                 default=['python', 'reactjs', 'javascript', 'scikit', 'c', 'cpp', 'sqlite', 'pytorch'])
    ...
    st.markdown('<br>', unsafe_allow_html=True)
    img_url = st.text_input(label='Enter Banner Image to be added at the top of the README',
                            value='https://cdn.pixabay.com/photo/2018/01/14/23/12/nature-3082832_1280.jpg')
    c1, c2 = st.columns(2)
    ...
    if save:
        code = default_html(name=name, github_username=github, waka_userName=waka_userid, linkedin_url=linkedin, p1=p1,
                            p2=p2, p3=p3, p4=p4, skills=user_skills, twitter_url=twitter, medium_url=medium,
                            portfolio_url=portfolio, isWaka=isWaka, github_stats_theme=github_stats_theme,
                            isJoke=isJoke,
                            joke_theme=joke_theme, img_url=img_url, img_width=img_width, img_height=img_height,
                            github_stats_type=github_stats_type, isBlog=isBlog)
